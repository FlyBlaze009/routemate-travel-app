...
const Navbar = ({ session }: { session: Session | null }) => {
  return (
    <nav className="bg-white shadow-md py-4 border-b border-gray-300">
      <div className="container mx-auto flex justify-between items-center px-6 lg:px-8">
        <Link href={'/'} className="flex justify-center items-center">
          <Image src={'/logo.png'} alt="logo" width={50} height={50} />
          <span className="text-2xl font-bold  text-gray-800">RouteMate</span>
        </Link>
        ...
        <div className="flex items-center space-x-4">
          {session ? (
            <>
              ...
              <button
                className="flex items-center justify-center cursor-pointer bg-red-400 hover:bg-red-600 text-white p-2 rounded-sm"
                onClick={logout}
              >
                Sign Out
              </button>
